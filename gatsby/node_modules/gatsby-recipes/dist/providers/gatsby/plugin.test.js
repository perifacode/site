...
const name = `gatsby-source-filesystem`; // Some of these are slow tests, because they hit the network
...
async function testPluginResource(root) {
  ...
  expect(createPlan).toMatchSnapshot(`GatsbyPlugin create plan`); // Test creating the resource

  const createResponse = await plugin.create(context, initialObject);
  const validateResult = Joi.validate(createResponse, { ...plugin.schema,
    ...resourceSchema
  });
  expect(validateResult.error).toBeNull();
  expect(createResponse).toMatchSnapshot(pluginSnapshotMatcher, `GatsbyPlugin create`); // Test reading the resource
  ...
  expect(updateResponse).toMatchSnapshot(pluginSnapshotMatcher, `GatsbyPlugin update`);
  const destroyReponse = await plugin.destroy(context, updateResponse);
  expect(destroyReponse).toMatchSnapshot(`GatsbyPlugin destroy`); // Ensure that resource was destroyed
...
describe(`gatsby-plugin resource`, () => {
  ...
  test(`all returns plugins as array`, async () => {
    const result = await plugin.all({
      root: STARTER_BLOG_FIXTURE
    });
    result.forEach(res => {
  ...
  test(`removes plugins by name and key`, async () => {
    ...
    configSrc = removePluginFromConfig(configSrc, {
      key: `special-key`
    });
