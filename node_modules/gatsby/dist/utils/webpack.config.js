...
const {
  CoreJSResolver
} = require(`./webpack/corejs-resolver`);
...
module.exports = async (program, directory, suppliedStage, port, {
  ...
  function getPlugins() {
    ...
    switch (stage) {
      case `develop`:
        configPlugins = configPlugins.concat([process.env.GATSBY_HOT_LOADER === `fast-refresh` && plugins.fastRefresh(), plugins.hotModuleReplacement(), plugins.noEmitOnErrors(), plugins.eslintGraphqlSchemaReload()]).filter(Boolean);
        break;
      ...
      case `develop`:
        {
          ...
          if (!(0, _localEslintConfigFinder.hasLocalEslint)(program.directory)) {
  ...
  function getResolve(stage) {
    ...
    const resolve = {
      ...
      alias: {
        ...
        "@pmmmwh/react-refresh-webpack-plugin": path.dirname(require.resolve(`@pmmmwh/react-refresh-webpack-plugin/package.json`)),
        "socket.io-client": path.dirname(require.resolve(`socket.io-client/package.json`)),
    ...
    if (target === `web`) {
      ...
      resolve.alias = Object.assign({}, {
        ...
        // for browser bundles
        "@reach/router": path.join(path.dirname(require.resolve(`@reach/router/package.json`)), `es`),
  ...
  if (stage === `build-html` || stage === `develop-html`) {
    ...
    const externalList = [`@reach/router/lib/history`, `@reach/router`, `common-tags`, `crypto`, `debug`, `fs`, `https`, `http`, `lodash`, `path`, `semver`, /^lodash\//, `zlib`]; // Packages we want to externalize because meant to be user-provided
